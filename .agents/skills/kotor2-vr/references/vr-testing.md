...
```bash
npm run vr:check   # 25 automated checks under an emulated Quest 3 — the gate
npm run vr:play    # launch a fresh Chrome profile for the real headset, CDP on :9422
```
...
## Manual headset rounds

Headset acceptance runs as numbered **rounds** against one persistent artifact,
"Headset Verification Run":
https://claude.ai/code/artifact/6897f1b9-af6a-4bea-8fd4-c693ce271af6

- It declares the `db` capability. Each round writes its own collection —
  `checks-rN` for per-check pass/fail/note and `session/log-rN` for the free-text
  log — so old rounds stay readable. Read results back with `read_db`. Bump
  `DB_COLLECTION`, `LOG_DOC` and the localStorage key together, or the new round
  opens pre-filled with the last one.
- **Prune between rounds.** Allen asked for it explicitly: remove checks that
  passed, carry the last failure note inline under each survivor, and add the
  new round's fixes as their own section. The list should shrink.
- Allen often reports in chat instead of the form (round 4 recorded one result
  in the db and a dozen in chat). Reconcile both before building the next round.
- **Setup order:** save any uncommitted working set, bring in merged branches,
  `npx tsc`, `npx jest --ci --silent`, `npm run webpack:dev`, `npm run vr:check`,
  then `npm run vr:play` and attach the CDP console tailer. `vr:check` and
  `vr:play` both bind port 8479, so never overlap them.
- Pick the save per section: `peragus two` (101PER, the Exile — humanoid hands,
  mining droids) and `ebon hawk start` (001EBO, T3-M4 — droid presentation,
  training droids). Saves live in `%LOCALAPPDATA%\Kotor2VR\saves`; the name is
  inside `savenfo.res`, not the folder name.
- When a fix cannot be settled offline, ship it with a `TEMPORARY` log line and
  mark its check "capture the console". Make sure the diagnostic can see the
